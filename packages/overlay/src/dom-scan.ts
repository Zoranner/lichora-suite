...
import { syncHostSurfaceTransparency } from "./host-surface";

export function scanOwnershipAttributeRegistrations(): RegionRegistration[] {
    const currentDocument = globalThis.document;
    if (!currentDocument?.querySelectorAll) {
        syncHostSurfaceTransparency([]);
        return [];
    }

    const registrations = [...currentDocument.querySelectorAll('[data-lichora="host"], [data-lichora="web"]')]
        .map((element) => toOwnershipRegistration(element))
        .filter((registration): registration is RegionRegistration => registration !== null);
    syncHostSurfaceTransparency(
        registrations
            .filter((registration) => registration.owner === "host")
            .map((registration) => registration.element),
    );
