...
当前 Rust IPC core 已实现 `OverlayPassMap` 和 `InputOwnershipMap` typed output 编解码与 golden test。Unity host 已解码并消费 `InputOwnershipMap`，并保留 `OverlayPassMap` 兼容读取。Web SDK 已能生成 ownership map，并通过临时 console bridge 交给 Rust 转换为 typed `InputOwnershipMap`。正式 CEF message route 或 process message bridge、Unity 内 DOM ownership 区域实测仍属于后续验收项。
