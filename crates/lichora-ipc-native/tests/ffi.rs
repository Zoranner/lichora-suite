...
macro_rules! ffi_wrap {
    ($(fn $name:ident($($arg:ident: $ty:ty),*) -> $ret:ty;)*) => {
        $(
            ...
            fn $name($($arg: $ty),*) -> $ret {
                unsafe { lichora_ipc_native::$name($($arg),*) }
            }
        )*
    };
}
...
fn session_open_creates_session_control_and_status_channel_files() {
    ...
    assert_eq!(result, EBI_OK);
    assert!(temp.path().join("Lichora_session-42_session").exists());
    assert!(temp.path().join("Lichora_session-42_control").exists());
    assert!(temp.path().join("Lichora_session-42_status").exists());
    assert_eq!(ebi_session_close(handle), EBI_OK);
    std::env::remove_var("EBI_IPC_DIR");
}
